...
func (d *driver) Run(c *execdriver.Command, pipes *execdriver.Pipes, startCallback execdriver.StartCallback) (execdriver.ExitStatus, error) {
	...
	if pipes.Stdin != nil {
		...
		if err != nil {
			logrus.Errorf("stdin failed to listen on %s err=%s", createProcessParms.StdInPipe, err)
			return execdriver.ExitStatus{ExitCode: -1}, err
		}
	...
	if err != nil {
		logrus.Errorf("stdout failed to listen on %s err=%s", createProcessParms.StdOutPipe, err)
		return execdriver.ExitStatus{ExitCode: -1}, err
	}
	...
	if !c.ProcessConfig.Tty {
		...
		if err != nil {
			logrus.Errorf("stderr failed to listen on %s err=%s", createProcessParms.StdErrPipe, err)
			return execdriver.ExitStatus{ExitCode: -1}, err
		}
