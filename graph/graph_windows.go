...
func (graph *Graph) storeImage(img *Image, layerData archive.ArchiveReader, root string) (err error) {
	...
	if layerData != nil && img.Parent != "" {
		if wd, ok := img.graph.driver.(*windows.WindowsGraphDriver); ok {
			var ids []string
			if img.Parent != "" {
				parentImg, err := img.graph.Get(img.Parent)
				if err != nil {
					return err
				}

				ids, err = img.graph.ParentLayerIds(parentImg)
				if err != nil {
					return err
				}
			}
			...
			return fmt.Errorf("Unable to find Windows driver.")
		}
	}
...
func (graph *Graph) TarLayer(img *Image) (arch archive.Archive, err error) {
	...
	return nil, fmt.Errorf("Unable to find Windows driver.")
}
